...
interface RubyBotEditorClientProps {
  botId: string;
}
...
export default function RubyBotEditorClient({
  botId,
}: RubyBotEditorClientProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/bots/${botId}`, {
        ...
        body: JSON.stringify({ code, language: "ruby" }),
      });
      if (!res.ok) {
        const message = await res.text();
