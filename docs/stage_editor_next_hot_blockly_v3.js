(() => {
  ...
  function registerHotBlocks() {
    ...
    const definitions = [
      ...
      { type: 'chaser_hot_walk_last', message0: 'HOTが前に進んだ向きで歩く（最初は %1）', args0: [{ type: 'field_dropdown', name: 'DIR', options: DIRS_UDLR }], previousStatement: null, nextStatement: null, colour: HOT_COLOR },
      { type: 'chaser_hot_walk_random', message0: 'HOTがどこかに歩く', previousStatement: null, nextStatement: null, colour: HOT_COLOR },
      { type: 'chaser_hot_put', message0: 'HOTがブロックを置く %1', args0: [{ type: 'field_dropdown', name: 'DIR', options: DIRS_UDLR }], previousStatement: null, nextStatement: null, colour: HOT_COLOR },
    ...
    generator.forBlock.chaser_hot_walk_last = (block) => `api.walkLast('${block.getFieldValue('DIR') || 'Up'}');`;
    ...
    generator.forBlock.chaser_hot_put = (block) => `api.put${block.getFieldValue('DIR') || 'Right'}();`;
    ...
    return {
      ...
      contents: [
        { kind: 'category', name: 'HOTの行動', colour: HOT_COLOR, contents: [
          ...
          { kind: 'block', type: 'chaser_hot_walk_last' },
          { kind: 'block', type: 'chaser_hot_walk_random' },
          { kind: 'block', type: 'chaser_hot_put' },
  ...
  function makeApi(play) {
    ...
    const setWalk = (direction) => { play.hotPending = { kind: 'walk', dir: direction }; };
    const setPut = (direction) => { play.hotPending = { kind: 'put', dir: direction }; };
    ['Up', 'Down', 'Left', 'Right'].forEach((direction) => {
      api[`walk${direction}`] = () => setWalk(direction);
      api[`put${direction}`] = () => setPut(direction);
    });
    api.walkLast = (fallback) => setWalk(play.hotLastDirection || fallback || 'Up');
    return api;
  }

  function applyHotAction(play) {
    ...
    if (action.kind === 'walk') {
      ...
      if (play.cool && play.cool.x === x && play.cool.y === y) {
        play.hot = { x, y };
        play.hotLastDirection = action.dir;
    ...
    if (action.kind === 'put') {
      if (!inside(play, x, y)) {
        play.logs.push(`${play.turn}: HOT put${action.dir} / outside`);
      } else if (play.cool && play.cool.x === x && play.cool.y === y) {
        play.logs.push(`${play.turn}: HOT put${action.dir} / COOL position`);
      } else if (play.tiles[y][x] === '.') {
        play.tiles[y][x] = '#';
        play.logs.push(`${play.turn}: HOT put${action.dir}`);
      } else {
        play.logs.push(`${play.turn}: HOT put${action.dir} / blocked`);
      }
      judge();
    }
  }
  ...
  function renderHotTab() {
    ...
    main.innerHTML = `<div class="tabs">${TABS.map(([id, label]) => `<div class="tab ${st.tab === id ? 'active' : ''}" data-tab="${id}">${label}</div>`).join('')}</div><div id="body"><div class="panel"><div class="grid"><div class="field"><label>HOTを動かす</label><label style="display:flex;align-items:center;gap:8px;margin-top:7px"><input id="hot-enabled" type="checkbox" ${behavior.enabled ? 'checked' : ''}>このステージでHOTの行動プログラムを有効にする</label></div><div class="field"><label>現在のマップ</label><div class="hint">${hasHot ? 'このマップには HOT（H）があります。' : 'このマップには H がありません。マップ編集で H を配置してください。'}</div></div></div><div class="hint">COOLの行動後、ゲームが続いている場合にHOTが1回行動します。HOTのブロック設置はCOOLやアイテムのあるマスには行えません。</div></div><div class="panel" style="margin-top:12px"><div class="section" style="margin-top:0">HOTの行動プログラム</div><div id="hot-blockly" style="height:540px;width:100%"></div></div></div>`;
  ...
  setGlobal('renderMain', function renderMainWithHot(...args) {
    if (st.tab !== 'hot') return previousRenderMain(...args);
    renderHotTab();
  });
